...
The goal of this project is to eliminate the need to manage dozens of scripts (like `.bat` or `.sh` files) or memorize complex command-line parameters. Upon startup, LMS offers an interactive terminal menu where you can choose the execution environment (Web or CLI), setup the environment automatically, download new models, and load them into memory with full GPU support.

---

### Features

* **Auto-Setup for `llama.cpp`:** If you don't have the binaries, LMS connects to the official `ggml-org/llama.cpp` GitHub API and automatically downloads and extracts the latest release for your OS!
* **Model Downloader:** Built-in HTTP downloader with a beautiful progress bar. You can choose from a curated list of high-quality models (like Phi-3 or Llama-3) or paste a direct `.gguf` URL.
* **Interactive Menu:** Dynamically select the interface and the model directly from the terminal using arrow keys.
* **GPU Acceleration:** LMS asks if you want to use your dedicated graphics card, automatically injecting the `-ngl 99` flags for maximum performance.
* **Dynamic Context Window:** Easily customize the AI's memory limit directly in the prompt before launch.
...
* **Clean Architecture:** Built from the ground up using SOLID principles, Strategy Patterns, and robust error handling in Rust.

---

### Default Directory Structure

If you do not provide any command-line parameters, the executable will look for (or create) the following structure in the same directory where it is launched:
